...
export const handler = async (event) => {
  ...
  try {
    ...
    } else if (method === 'GET' && path === '/videos') {
      return await listVideos(event);
    } else if (method === 'GET' && path.startsWith('/videos/') && !path.endsWith('/process')) {
      const videoId = pathParams.id || path.split('/')[2];
      return await getVideo(videoId);
    } else if (method === 'POST' && path.startsWith('/videos/') && path.endsWith('/process')) {
      const videoId = pathParams.id || path.split('/')[2];
